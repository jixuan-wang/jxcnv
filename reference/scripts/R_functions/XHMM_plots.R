XHMM_plots <- function(PLOT_PATH, JOB_PREFICES, JOB_TARGETS_TO_GENES=NULL, SAMPLE_FEATURES=NULL, SQ_THRESH=60, NUM_ADD_TARGS=2, PLOT_READ_DEPTHS=TRUE, PLOT_PC_CORRS=TRUE, PLOT_ALL_CNVS=TRUE, USE_XCNV_TO_PLOT=NULL, INCLUDE_PEDIGREE_SAMPLES=NULL, PLOT_ONLY_PNG=TRUE, LIMIT_MEMORY=FALSE, MIN_PC_SD_TO_PLOT=1) {
	...
	if (PLOT_PC_CORRS) {
		...
		for (job in JOBS) {
			...
			USE_SAMPLE_FEATURE_NAMES[[job]] = c()
			for (sampVal in SAMPLE_FEATURE_NAMES) {
				dat = xhmm_data[[job]][[sampVal]][PC_LOADINGS_samps]
				if (var(dat, na.rm=TRUE) > 0) {
					USE_SAMPLE_FEATURE_NAMES[[job]] = c(USE_SAMPLE_FEATURE_NAMES[[job]], sampVal)
				}
			}
		
			vals = matrix(0, nrow=length(PC_LOADINGS_samps), ncol=length(USE_SAMPLE_FEATURE_NAMES[[job]]), dimnames=list(PC_LOADINGS_samps, USE_SAMPLE_FEATURE_NAMES[[job]]))
			for (sampVal in USE_SAMPLE_FEATURE_NAMES[[job]]) {
				vals[, sampVal] = xhmm_data[[job]][[sampVal]][PC_LOADINGS_samps]
			}

			# Add back in 0 correlation for last PC's loadings:
			PC_LOADINGS.corr[[job]] = rbind(cor(PC_LOADINGS_t, vals, use="na.or.complete"), rep(0, length(USE_SAMPLE_FEATURE_NAMES[[job]])))
		}
		...
		for (job in JOBS) {
			...
			featureNames = c(TARGET_FEATURE_NAMES, USE_SAMPLE_FEATURE_NAMES[[job]])
		...
		for (job in JOBS) {
			...
			SDdata = xhmm_data[[job]][["PC_SD"]]
		        ...
		        useInds = which(SDdata >= MIN_PC_SD_TO_PLOT)
