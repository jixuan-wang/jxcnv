...
phenotypeDataToBinarySampleProperties <- function(phenotypes, ADD_EXCLUDE_COLS=c()) {
	BINARIZE_TYPES = c("integer", "character")
	VAL_NAME_SEP = "_"

	EXCLUDE_COLS = union(ADD_EXCLUDE_COLS, c("#ID", "FID", "IID"))
	...
	for (col in propertyCols) {
		if (colTypes[col] %in% BINARIZE_TYPES) {
			...
			quantVar = phenotypes[, col]
			names(quantVar) = SAMPLES
			binaryProperties[[col]] = quantVar
		}
	}
